...
/*   Updated: 2025/10/25 17:03:50 by fernafer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_calculate_isos(t_fdf *fdf)
{
	...
	while (y < fdf->height)
	{
		...
		while (x < fdf->width)
		{
			...
			fdf->matrix[y][x].xiso = (x - y) * cos(angle)
				* fdf->data_cam->zoom + fdf->data_cam->x_off;
			fdf->matrix[y][x].yiso = ((x + y) * sin(angle) - z)
				* fdf->data_cam->zoom + fdf->data_cam->y_off;
			x++;
		}
		y++;
	}
...
void	ft_draw_map(t_fdf *fdf)
{
	...
	while (y < fdf->height)
	{
		...
		while (x < fdf->width)
		{
			ft_put_pixel(fdf->data_img, fdf->matrix[y][x].xiso,
				fdf->matrix[y][x].yiso, 0xFFFFFF);
			x++;
		}
		y++;
	}
